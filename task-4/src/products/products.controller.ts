...
export class ProductsController {
  ...
  @Get('search')
  search(@Query('keyword') keyword: string) { return this.productsService.search(keyword); }

  @Get('category/:cat')
  findByCategory(@Param('cat') cat: string) { return this.productsService.findByCategory(cat); }
  ...
  @Patch(':id/toggle')
  toggle(@Param('id', ParseIntPipe) id: number) { return this.productsService.toggleActive(id); }
}